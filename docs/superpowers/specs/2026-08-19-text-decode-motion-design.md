...
A semantic trigger may cache a pending user snapshot only when its generation and live media conditions match the intent. Multiple reports from that input token can refine the snapshot, but a later responsive generation's placeholder or update cannot consume the old intent or overwrite an already confirmed snapshot. When the user remains inside the same chapter and no semantic trigger fires, the guarded scroll listener coalesces layout sampling to one animation frame and updates that token's snapshot from the current canonical anchor, viewport offset, and `window.scrollY`. The winning restoration token clears active intent before refreshing ScrollTrigger, so refresh-generated reports cannot overwrite the user snapshot. Restoration prefers that pending snapshot, clears it atomically with the transition state, and otherwise retains the original locked snapshot and exact offset. Scroll events without a valid input token never establish user intent, so ScrollTrigger teardown, refresh, programmatic restoration, stale generations, and synthetic layout shifts cannot replace canonical state.
...
- Preserve real wheel, touch-move, and scrolling-key intent during the guarded rebuild with generation-bound tokens, current-context semantic reports, and a coalesced same-chapter position capture; never report or play it before restoration unlocks.
